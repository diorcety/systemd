...
int enroll_tpm2(struct crypt_device *cd,
        ...
        size_t secret_size, secret2_size, blob_size, hash_size, srk_buf_size = 0;
        ...
        r = tpm2_seal(device, pcr_mask, &secret, &secret_size, &blob, &blob_size, &hash, &hash_size, &pcr_bank, &primary_alg, &srk_buf, &srk_buf_size);
        if (r < 0)
                return r;
        ...
        r = tpm2_unseal(device, pcr_mask, pcr_bank, primary_alg, blob, blob_size, hash, hash_size, srk_buf, srk_buf_size, &secret2, &secret2_size);
        if (r < 0)
                return r;
        ...
        r = tpm2_make_luks2_json(keyslot, pcr_mask, pcr_bank, primary_alg, blob, blob_size, hash, hash_size, srk_buf, srk_buf_size, &v);
        if (r < 0)
