...
static int tpm2_make_primary(
        ...
        if (use_srk_model) {
                rc = sym_Esys_EvictControl(c, ESYS_TR_RH_OWNER, primary,
                                ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE, SRK_HANDLE, &primary);
                if (rc != TSS2_RC_SUCCESS)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
...
int tpm2_get_good_pcr_banks_strv(
        ...
        for (int i = 0; i < n_algs; i++) {
                ...
                n = strdup(EVP_MD_name(implementation));
                if (!n)
                        return log_oom();
...
int tpm2_seal(
        ...
        r = tpm2_make_primary(c.esys_context, /* alg = */0, !!ret_srk_buf, &primary_alg, &primary);
        if (r < 0)
                return r;
        ...
        r = tpm2_make_pcr_session(c.esys_context, primary, session, pcr_mask, UINT16_MAX, NULL,
...
int tpm2_unseal(
        ...
        r = tpm2_make_primary(c.esys_context, primary_alg, false, NULL, &primary);
        if (r < 0)
                return r;
        ...
        r = tpm2_make_pcr_session(c.esys_context, primary, hmac_session, pcr_mask, pcr_bank, &session,
                                  &policy_digest, NULL);
        if (r < 0)
                goto finish;
